...
def get_openai_client(base_url: str, api_key: str) -> AsyncOpenAI:
    ...
    if cache_key not in _openai_clients:
        _openai_clients[cache_key] = AsyncOpenAI(
            ...
            timeout=float(os.environ.get("API_TIMEOUT", getattr(Config, "API_TIMEOUT", 600.0)))
        )
...
class UnifiedAgent:
    ...
    async def _call_gemini(self, prompt: str, system_instr: str) -> Any:
        ...
        if gemini_key not in _gemini_clients:
            _gemini_clients[gemini_key] = genai.Client(
                api_key=gemini_key,
                http_options={'timeout': float(os.environ.get("API_TIMEOUT", getattr(Config, "API_TIMEOUT", 600.0)))}
            )
